...
function between (min, max) {
  return {
    ...
    $message: _ref => {
      ...
      return `该值需要在${$params.min}和${$params.max}之间`;
    },
    $params: {
      min,
      max,
...
function or () {
  return withParams({
    type: 'or'
  }, withMessage('无匹配项', or$1(...arguments)));
}
...
function and () {
  return withParams({
    type: 'and'
  }, withMessage('没有完全匹配', and$1(...arguments)));
}

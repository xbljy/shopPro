...
export default {
  ...
  setup() {
    ...
    const products = computed(() => store.getters["products/getVegetablesProducts"]);
    ...
    const loadData = async () => {
      ...
      if (Object.keys(products.value).length == 0)
        await store.dispatch("products/fetchProductsToCollection", "vegetables");
    ...
    const getNumberOfProductsInCollection = computed(() =>
      store.getters["products/getNumberOfProductsInCollection"]("vegetables")
    );

...
export default {
  ...
  setup() {
    ...
    const products = computed(() => store.getters["products/getFrozenfoodProducts"]);
    ...
    const loadData = async () => {
      ...
      if (Object.keys(products.value).length == 0)
        await store.dispatch("products/fetchProductsToCollection", "frozenfood");
    ...
    const getNumberOfProductsInCollection = computed(() =>
      store.getters["products/getNumberOfProductsInCollection"]("frozenfood")
    );
